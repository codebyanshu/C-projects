...
void appointments()
{
    ...
    case 1:
        ...
        scanf(" %[^\n]s", newAppointment.doctor);
        fflush(stdin);
        printf("Enter appointment date (DD/MM/YYYY): ");
        scanf(" %[^\n]s", newAppointment.appointmentDate);
        fptr = fopen("appointments.txt", "a");
        if (fptr != NULL) {
            fprintf(fptr, "Name : %s,\nAge : %d,\nId : %d,\nDoctor : %s,\nDate : %s\n", newAppointment.name, newAppointment.age, newAppointment.id, newAppointment.doctor, newAppointment.appointmentDate);
            fclose(fptr);
    ...
    case 3:
        ...
        break;
    case 4:
        printf("Searching The Appointments\n");
        return;
    default:
        printf("Invalid choice. Please try again.\n");
        appointments();
